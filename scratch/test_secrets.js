...
async function run() {
  try {
    ...
    const tokenFallback = jwt.sign(payload, FALLBACK_SECRET, { expiresIn: '15m' });

    console.log('--- Testing Token with ENV_SECRET ---');
    try {
      const res = await axios.get('http://localhost:3000/api/users/discover', {
    ...
    console.log('--- Testing Token with FALLBACK_SECRET ---');
    try {
      const res = await axios.get('http://localhost:3000/api/users/discover', {
        headers: { Authorization: `Bearer ${tokenFallback}` }
      });
      console.log('🟢 FALLBACK_SECRET Succeeded!', res.status);
    } catch (e) {
      console.log('🔴 FALLBACK_SECRET Failed:', e.response?.status, e.response?.data);
    }
